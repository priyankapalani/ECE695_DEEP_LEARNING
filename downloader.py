...
parser.add_argument('-number_of_classes', default = 5, type=int)
...
parser.add_argument('-classes',default=['dog','cat','boat','truck','bicycle'],nargs='*',help="Ensure that the class_list provided has similar class names i.e dump truck, delivery turck etc will all be put under the class truck")
...
classes_to_scrape=['n03256166','n03173929','n04461696','n04095210','n03351262','n02835271','n02103841','n02109150','n02104184','n02104280','n02085936','n02085272','n02121808','n01884203','n04490091','n02858304','n02834778','n02084071','n02121620']
...
def run_train_test_split():
    data=["imagenet_train","imagenet_test"]
    for j in data:
        for i in classes:
            try:
                os.makedirs(j+"/"+i)
                ...
                pass
    for i in os.listdir(imagenet_images_folder):
        print("Class :"+i)
        images=os.listdir(os.path.join(imagenet_images_folder,i))
        count=len(images)
        print("Number of samples for "+i+"  :"+str(count))
        for j in range(random.randint(10,25)):
            random.shuffle(images)
        train_images=images[:int(0.85*count)]
        test_images=images[int(0.85*count):]
        print("Train samples for "+i+" "+str(len(train_images)))
        print("Test samples for "+i+" "+str(len(test_images)))
        for j in train_images:
            shutil.move(os.path.join(imagenet_images_folder,i,j),os.path.join("imagenet_train",i,j))
        for j in test_images:
            shutil.move(os.path.join(imagenet_images_folder,i,j),os.path.join("imagenet_test",i,j))
        assert len(train_images)==len(os.listdir(os.path.join("imagenet_train",i))), "Something went wrong in train image transfer"      
        assert len(test_images)==len(os.listdir(os.path.join("imagenet_test",i))), "Something went wrong in test image transfer"      
